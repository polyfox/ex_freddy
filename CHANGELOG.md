...
- Removed OpenTelemetry in favour of plain `:telemetry`
- Support `GenServer.server()` for connections in `Freddy.Core.Actor`, this allows supplying connections that may be registered in a Registry such as: `{:via, Registry, {MyRegistry, "other_connection"}}`
